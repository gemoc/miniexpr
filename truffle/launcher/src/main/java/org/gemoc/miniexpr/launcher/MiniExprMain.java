package org.gemoc.miniexpr.launcher;
...
public final class MiniExprMain {

    private static final String MNXPR = "mnxpr";
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            // @formatter:off
            source = Source.newBuilder(MNXPR, new InputStreamReader(System.in), "<stdin>").build();
            // @formatter:on
        } else {
            source = Source.newBuilder(MNXPR, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        ...
        try {
            context = Context.newBuilder(MNXPR).in(in).out(out).options(options).build();
